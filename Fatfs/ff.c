...
//#include "stm32f10x.h"	/* add by fire */
...
/*-----------------------------------------------------------------------*/
//注册或注销一个工作区  成功返回  FR_OK   即(0) /
/*f_mount 函数在 FatFs模块上注册/注销一个工作区。  在使用任何其他文件函数之前，
必须使用该函数为每个卷注册一个工作区。要注销一个工作区，只要指定FileSystemObject 
为NULL即可，然后该工作区可以被丢弃。 该函数只初始化给定的工作区，以及将该工作
区的地址注册到内部表中，不访问磁盘I/O 层。卷装入过程是在f_mount 函数后或存储介
质改变后的第一次文件访问时完成的。 */
FRESULT f_mount (
...
/*-----------------------------------------------------------------------*/
/*
 *如果函数成功，则创建一个文件对象。该文件对象被后续的读/ 写函数用来访问文件。
 *如果想要关闭一个打开的文件对象，则使用f_close 函数。如果不关闭修改后的文件，
 *那么文件可能会崩溃。 在使用任何文件函数之前，必须使用f_mount 函数为驱动器注
 *册一个工作区。只有这样，其他文件函数才能正常工作。
 */
FRESULT f_open (
	FIL *fp,			/* FileObject将被创建的文件对象结构的指针 */
	const XCHAR *path,	/* NULL结尾的字符串指针，该字符串指定了将被创建或打开的文件名 */
	BYTE mode			/* 指定文件的访问类型和打开方法。FA_READ或FA_WRITE等等 */
)
{
	FRESULT res;
	DIR dj;
...
/* Read File                                                             */
/*-----------------------------------------------------------------------*/
/*文件对象中的读/ 写指针以已读取字节数增加。该函数成功后，应该检查  *ByteRead  
 *来检测文件是否结束。在读操作过程中，一旦  *ByteRead < ByteToRead  ，则读/ 写
 *指针到达了文件结束位置。
 */
FRESULT f_read (
...
/* Write File                                                            */
/*-----------------------------------------------------------------------*/
/*文件对象中的读/ 写指针以已写入字节数增加。该函数成功后，应该检查  *ByteWritten  
来检测磁盘是否已满。在写操作过程中，一旦  *ByteWritten < *ByteToWritten  ，则意味着该卷已满*/
FRESULT f_write (
	FIL *fp,			/* 指向将被写入的已打开的文件对象结构的指针*/
	const void *buff,	/* 指向存储写入数据的缓冲区的指针*/
	UINT btw,			/* 要写入的字节数，UINT范围内 */
	UINT *bw			/* 指向返回已写入字节数的UINT变量的指针。在调用该函数后，无论结果如何，数值都是有效的 */
)
{
	FRESULT res;
	DWORD clst, sect;
...
/*-----------------------------------------------------------------------*/
/* Synchronize the File Object       //同步一个写文件的缓存信息                                    */
/*-----------------------------------------------------------------------*/
/*
 *f_sync 函数当_FS_READONLY == 0时可用。
 *f_sync 函数和f_close 函数执行同样的过程，但是文件仍处于打开状态，并且可以
 *继续对文件执行读/ 写/ 移动指针操作。这适用于以写模式长时间打开文件，比如
 *数据记录器。定期的或f_write 后立即执行f_sync 可以将由于突然断电或移去磁盘
 *而导致数据丢失的风险最小化。在f_close 前立即执行f_sync 没有作用，因为在
 *f_close 中执行了f_sync 。换句话说，这两个函数的差异就是文件对象是不是无效的。 
*/
FRESULT f_sync (
	FIL *fp		/* Pointer to the file object */
)
...
/* Close File                                                            */
/*-----------------------------------------------------------------------*/
/*f_close 函数关闭一个打开的文件对象。无论向文件写入任何数据，文件的缓存信息
都将被写回到磁盘。该函数成功后，文件对象不再有效，并且可以被丢弃。如果文件对
象是在只读模式下打开的，不需要使用该函数，也能被丢弃。*/
FRESULT f_close (
...
/*-----------------------------------------------------------------------*/
/* Change Current Drive/Directory       改变当前驱动器                                 */
/*-----------------------------------------------------------------------*/

#if _FS_RPATH
/*
 *f_chdrive 函数改变当前驱动器。当前驱动器号初始值为 0，注意：当前驱动器被保
 *存为一个静态变量，因此它也影响使用文件函数的其它任务。
 */
FRESULT f_chdrive (
...
#if _FS_MINIMIZE <= 2		//f_lseek 函数当FS_MINIMIZE <= 2 时可用。
/*-----------------------------------------------------------------------*/
/* Seek File R/W Pointer                                                 */
/*-----------------------------------------------------------------------*/
/*	offset 只能被指定为相对于文件起始处的字节数。当在写模式下指定了一个超过文
 *	件大小的 offset 时，文件的大小将被扩展，并且该扩展的区域中的数据是未定义的。
 *	这适用于为快速写操作迅速地创建一个大的文件。f_lseek 函数成功后，为了确保读
 *	写指针已被正确地移动，必须检查文件对象中的成员fptr 。如果fptr 不是所期望的
 *	值，则发生了下列情况之一。 
 *	1   文件结束。指定的offset 被钳在文件大小，因为文件已被以只读模式打开。 
...
#if _FS_MINIMIZE <= 1		//f_opendir 函数当_FS_MINIMIZE <= 1 时可用。
/*-----------------------------------------------------------------------*/
/* Create a Directroy Object     //打开一个目录                                        */
/*-----------------------------------------------------------------------*/
/*
 *f_opendir 函数打开一个已存在的目录，并为后续的调用创建一个目录对象。该目
 *录对象结构可以在任何时候不经任何步骤而被丢弃。
 */
FRESULT f_opendir (
	DIR *dj,			/* 目录对象结构的指针*/
	const XCHAR *path	/* 目录名的指针*/
)
{
	FRESULT res;
	NAMEBUF(sfn, lfn);
...
/*-----------------------------------------------------------------------*/
/* Read Directory Entry in Sequense              //读取目录项                        */
/*-----------------------------------------------------------------------*/
/*
 *f_readdir 函数顺序读取目录项。目录中的所有项可以通过重复调用 f_readdir 
 *函数被读取。当所有目录项已被读取并且没有项要读取时，该函数没有任何错误
 *地返回一个空字符串到 f_name[] 成员中。当 Fno给定一个空指针时，目录对象
 *的读索引将被回绕。 
 *当LFN 功能被使能时，在使用 f_readdir 函数之前，文件信息结构中的 lfname 
 *和lfsize 必须被初始化为有效数值。lfname 是一个返回长文件名的字符串缓冲
 *区指针。lfsize 是以字符为单位的字符串缓冲区的大小。如果读缓冲区或 LFN 
 *工作缓冲区的大小(对于LFN) 不足，或者对象没有LFN ，则一个空字符串将被返
 *回到 LFN 读缓冲区。如果LFN 包含任何不能被转换为 OEM代码的字符，则一个空
 *字符串将被返回，但是这不是Unicode API配置的情况。当lfname 是一个空字符
 *串时，没有LFN 的任何数据被返回。当对象没有LFN 时，任何小型大写字母可以
 *被包含在SFN 中。 当相对路径功能被使能(_FS_RPATH == 1)时，"."和".."目录
 *项不会被过滤掉，并且它将出现在读目录项中。
*/
FRESULT f_readdir (
	DIR *dj,			/*指向打开的目录对象结构的指针*/
	FILINFO *fno		/* 指向文件信息结构的指针*/
)
{
	FRESULT res;
	NAMEBUF(sfn, lfn);
...
#if _FS_MINIMIZE == 0		//f_stat 函数当_FS_MINIMIZE == 0 时可用。
/*-----------------------------------------------------------------------*/
/* Get File Status                                                       */
/*-----------------------------------------------------------------------*/
/*f_stat 函数获取一个文件或目录的信息。信息的详情，请参考FILINFO结构和f_readdir 函数。*/
FRESULT f_stat (
	const XCHAR *path,	/* 文件名或目录名的指针*/
...
#if !_FS_READONLY	//f_getfree 函数当_FS_READONLY == 0并且_FS_MINIMIZE == 0 时有效。
/*-----------------------------------------------------------------------*/
/* Get Number of Free Clusters            //获取空闲簇的数目                                */
/*-----------------------------------------------------------------------*/
/*
 *f_getfree 函数获取驱动器上空闲簇的数目。文件系统对象中的成员 csize 是每簇
 *中的扇区数，因此，以扇区为单位的空闲空间可以被计算出来。当FAT32 卷上的
 *FSInfo 结构不同步时，该函数返回一个错误的空闲簇计数。 
 */

FRESULT f_getfree (
	const XCHAR *path,	/*驱动器的根目录  */
	DWORD *nclst,		/* 存储空闲簇数目变量的指针 */
	FATFS **fatfs		/* 文件系统对象指针的指针*/
)
{
	FRESULT res;
...
/*-----------------------------------------------------------------------*/
/* Truncate File               //截断文件大小                                          */
/*-----------------------------------------------------------------------*/
/*
 *f_tr uncate 函数当_FS_READONLY == 0  并且  _FS_MINIMIZE == 0 时可用。
 *f_truncate 函数截断文件到当前的文件读/ 写指针。当文件读/ 写指针已经指向文
 *件结束时，该函数不起作用。
 */
FRESULT f_truncate (
	FIL *fp		/* Pointer to the file object */
...
/*-----------------------------------------------------------------------*/
/* Delete a File or Directory       //移除一个对象                                      */
/*-----------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------*/
/* Change Timestamp            //函数修改一个文件或目录的时间戳。                                         */
/*-----------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------*/
/* Rename File/Directory                    //重命名一个对象                             */
/*-----------------------------------------------------------------------*/
/*
 *f_rename 函数重命名一个对象，并且也可以将对象移动到其他目录。逻辑驱动器号
 *由原名决定，新名不能包含一个逻辑驱动器号。不要重命名打开的对象。
 */
FRESULT f_rename (
...
/*
 *f_forward函数从文件中读取数据并将数据转发到输出流，而不使用数据缓冲区。这
 *适用于小存储系统，因为它在应用模块中不需要任何数据缓冲区。文件对象的文件指
 *针以转发的字节数增加。如果*ByteFwd < ByteToFwd 并且没有错误，则意味着由于
 *文件结束或在数据传输过程中流忙，请求的字节不能被传输。
 */
FRESULT f_forward (
	FIL *fp, 						/* 文件对象  */
...
/*-----------------------------------------------------------------------*/
/* Create File System on the Drive        //在驱动器上创建一个文件系统                           */
/*-----------------------------------------------------------------------*/
...
/*
 *f_mkfs 函数在驱动器中创建一个 FAT 文件系统。对于可移动媒介，有两种分区规则：
 *FDISK 和SFD ，通过参数PartitioningRule选择。FDISK 格式在大多数情况下被推荐
 *使用。该函数当前不支持多分区，因此，物理驱动器上已存在的分区将被删除，并且
 *重新创建一个占据全部磁盘空间的新分区。 根据Microsoft 发布的FAT 规范，FAT 
 *分类：FAT12/FAT16/FAT32 ，由驱动器上的簇数决定。因此，选择哪种FAT 分类，取
 决于卷大小和指定的簇大小。簇大小影响文件系统的性能，并且大簇会提高性能。
*/

FRESULT f_mkfs (
	BYTE drv,			/* 逻辑驱动器号(0 - 9) */
	BYTE partition,		/*分区规则 0:FDISK, 1:SFD */
						/*PartitioningRule 当给定0 时，首先在驱动器上的第一个*/
						/*扇区创建一个分区表，然后文件系统被创建在分区上。这*/
						/*被称为 FDISK格式化，用于硬盘和存储卡。当给定1 时，文*/
						/*件系统从第一个扇区开始创建，而没有分区表。这被称为超*/
						/*级软盘(SFD)格式化，用于软盘和可移动磁盘。*/
	WORD allocsize		/* 分配单元大小 [bytes] */
)
{
...
#if _USE_STRFUNC
/*-----------------------------------------------------------------------*/
/* Get a string from the file      从文件中读取一个字符串                                      */
/*-----------------------------------------------------------------------*/
/*
 *f_gets 函数当_USE_STRFUNC == 1 或者_USE_STRFUNC == 2 时可用。如果_USE_STRFUNC == 2，
 *文件中包含的'\ r' 则被去除。 f_gets 函数是f_read 的一个封装函数。当读取到'\ n' 、文
 *件结束或缓冲区被填冲了 Size-1 个字符时，读操作结束。读取的字符串以'\ 0' 结束。当文
 *件结束或读操作中发生了任何错误，f_gets()返回一个空字符串。可以使用宏 f_eof() 和f_error() 
 *检查EOF 和错误状态。
 */
char* f_gets (
	char* buff,	/* 读缓冲区 */
...
/*-----------------------------------------------------------------------*/
/* Put a string to the file        向文件中写入一个字符串                */
/*-----------------------------------------------------------------------*/
/*
 *f_puts()当(_FS_READONLY == 0)&&(_USE_STRFUNC == 1 || _USE_STRFUNC == 2) 时可用。
 *当_USE_STRFUNC == 2 时，字符串中的'\ n'被转换为"\ r\ n" 写入文件中。
 */
int f_puts (
...
/*-----------------------------------------------------------------------*/
/* Put a formatted string to the file 向文件中写入一个格式化字符串      */
/*-----------------------------------------------------------------------*/
/*
 *f_printf 函数当(_FS_READONLY == 0)&&(_USE_STRFUNC == 1 || _USE_STRFUNC == 2) 时可用。
 *当_USE_STRFUNC == 2 时，包含在格式化字符串中的'\ n' 将被转换成"\ r\ n" 写入文件中。 
 *f_printf 函数是f_putc 和f_puts 的一个封装函数。如下所示，格式控制符是标准库的一个子集： 
 *类型：c s d u X 
 *大小：l 
 *标志：0
 */
